...
public class SettingsUtil {
    ...
    public boolean updateProperties(String filename) throws IOException {
        ...
        try {
            try (InputStream inputStream = getClass().getResourceAsStream(filename)) {
                SettingsUtil.properties.load(inputStream);
            }
        } catch (Exception e) {
            return false;
        }
    ...
    private boolean saveStandardProperties(String filename) {
        SettingsUtil.properties.setProperty(SettingsUtil.JDBC_DRIVER_NAME, "org.sqlite.JDBC");
        SettingsUtil.properties.setProperty(SettingsUtil.VIEW_HEIGHT, "600");
        SettingsUtil.properties.setProperty(SettingsUtil.VIEW_WIDTH, "800");
        SettingsUtil.properties.setProperty(SettingsUtil.VIEW_X_POSITION, "0");
        SettingsUtil.properties.setProperty(SettingsUtil.VIEW_Y_POSITION, "0");
        SettingsUtil.properties.setProperty(SettingsUtil.DB_URL, "jdbc:sqlite:.geodaten.db3");
        SettingsUtil.properties.setProperty(SettingsUtil.DB_USER, "root");
        SettingsUtil.properties.setProperty(SettingsUtil.DB_PASSWORD, "");
        
        File file = new File(filename);
        
         try {
            OutputStream outputStream = new FileOutputStream(file);
            SettingsUtil.properties.store(outputStream, "Settings for SQLLiteBrowser");
        } catch (Exception e) {
            return false;
        }
        return true;
